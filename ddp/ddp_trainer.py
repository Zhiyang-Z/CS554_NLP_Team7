...
class Pre_Trainer:
    ...
    def train(self):
        ...
        for epoch in range(2000000000): # termination is decide by human.
            ...
            self.train_data_loader.dataset.dataset.shuffle(46+epoch)
            for _, data in tqdm(enumerate(self.train_data_loader)):
                ...
                # if step % 50000 == 0 and self.rank == 0:
    ...
    def sample(self, ini_input):
        ...
        ans_token = []
        next_token_logits = self.model(prompt)[0,-1,:]
        k = 50 # top k sample
        ...
        while not (len(ans_token) >= 2048 or ans_token[-1] == tokenizer.eos_token_id):
            last_token = torch.tensor([ans_token[-1]]).unsqueeze(0).to(self.device)
            next_token_logits = self.model(last_token)[0,-1,:]
            k = 50 # top k sample
            topk_logits, topk_indices = torch.topk(next_token_logits, k)
            topk_probs = torch.softmax(topk_logits, dim=-1)
            next_token = topk_indices[torch.multinomial(topk_probs, 1)].cpu().item()
